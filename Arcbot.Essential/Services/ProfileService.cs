...
namespace Arcbot.Essential.Services
{
    public class ProfileService
    {
            ...
            where TItem : Item
        {
            ...
            return profile == null
                ? default
                : (TItem) profile.Inventory.FirstOrDefault(x => x.GetType() == typeof(TItem));
        }
        ...
        public long CountCoin(User user)
        {
            return Count<Coin>(user);
        }

        public long TakeCoin(User user, long howMany)
        {
            return Take(user, new Coin(howMany));
        }

        public long PutCoin(User user, long howMany)
        {
            return Put(user, new Coin(howMany));
        }
    }
}